...
/*   Updated: 2015/10/22 16:37:23 by vmonteco         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int					uf_test_lstnew(void)
{
	...
	if (begin != NULL)
	{
		if (&v == begin->content)
			D_ERROR;
		ft_putnbr(*(size_t*)begin->content);
...
int				uf_test_strlcat(void)
{
	...
	ft_putendl(dest2);
	ft_putstr("result (j) : ");
	ft_putendl(ft_itoa(j));
	ft_putstr("result (k, ft) : ");
